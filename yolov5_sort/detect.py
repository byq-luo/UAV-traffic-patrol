...
def detect(save_img=False):
    ...
    (save_dir / 'det' if save_txt else save_dir).mkdir(
    ...
    for path, img, im0s, vid_cap in dataset:
        ...
        for i, det in enumerate(pred):  # detections per image
            ...
            txt_path = str(save_dir / 'det' / p.stem)  # img.txt
                ...
                for *xyxy, conf, cls in reversed(det):
                    ...
                    if save_txt:  # Write to file
                        ...
                        if cls not in [2, 5, 7]:
                            continue
                        line = (
                            frame, -1, *xywh, conf, -1, -1, -1, cls
                        )  # if opt.save_conf else (cls, *xywh)  # label format
                        with open(txt_path + '.txt', 'a') as f:
                            f.write(('%g' + ',%g' * (len(line) - 1)).rstrip() % line + '\n')
    ...
    if save_txt or save_img:
        s = f"\n{len(list(save_dir.glob('det/*.txt')))} labels saved to {save_dir / 'det'}" if save_txt else ''
